...
class environment2048(gym.Env):
    ...
    def reset(self):
        ...
        self._driver.find_element(By.XPATH, '//*[@id="ez-accept-all"]') \
            .click()

        self._driver.find_element(By.CLASS_NAME, "restart-button") \
            .click()
    ...
    def _next_observation(self):
        grid2 = np.zeros((4,4)) 
        parent = self._driver.find_element(By.XPATH, '/html/body/div[3]/div[3]/div[3]')
    ...
    def _get_score(self):
        try:
            num = int(''.join(self._driver.find_element(By.XPATH, '/html/body/div[3]/div[1]/div/div[1]').text))
        except:
            num = 0
        return num
    ...
    def _get_done(self):
        text = ''
        try:
            text = self._driver.find_element(By.XPATH, '/html/body/div[3]/div[3]/div[1]/div/a[2]').text
        except:
             pass
        if len(text) > 2:
            return True
    ...
    def step(self, action: int):
        ...
        if done:
            ...
            reward = 0.01
        try:
            num = int(''.join(self._driver.find_element(By.XPATH, '/html/body/div[3]/div[1]/div/div[1]/div').text))
        except:
            num = 0
        reward += 0.5*num
        '''try:
